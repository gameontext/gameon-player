...
then
  ...
  then
    ...
    curl -X 'POST' --silent --data-binary '{"text":"Docker Build for the player service has failed."}' $SLACK_WEBHOOK_PATH > /dev/null
    exit -2
  else
    ...
    ../docker run -d -p 9082:9080 -p 9443:9443 --restart=always --link etcd -e LICENSE=accept -e ETCDCTL_ENDPOINT=http://etcd:4001 --name=gameon-player gameon-player
    if [ $? != 0 ]
    then
      echo "Docker run failed.. it's too late.. the damage is done already."
      curl -X 'POST' --silent --data-binary '{"text":"Docker Run for the player service has failed."}' $SLACK_WEBHOOK_PATH > /dev/null
      exit -3
    else
      cd ..
      rm -rf dockercfg
